...
Furthermore, he gives valuable recommendations as to where Scala's strength resides.
...
What was initially thought of as a LinkedIn mind-dump, quickly escalated to the blog post you started reading.
...
Therefore, I provide the reader with a table of contents, so they jump ahead if they feel so inclined.
...
The streams API was mock of functional programming, Optional values raised questions like 
"but why can I just do `!= null`?" and people had to write `final` everywhere just to feel a bit more immutable.
...
I was lucky to have a colleague who felt excited as well while also having the support of management to try out 
something new.
...
improvement, and a stable product that succeeded my stay at the given employer.

## The fair
Hungry for the next Scala gig, I landed at a  company working with decent volumes of airfare observations.
...
As it was finding a venue and making appealing events.
...
This time around there was Kubernetes and many more people knowledgeable of Scala.
Akka was just another tool in the pipeline as were some Typelevel libraries.
...
and Scala on top made my success inevitable.
...
Old corporations on the other hand have tons of data.
Traditionally slow and bureaucratic, these giants are awakening to the power of cloud
and how small, high-skilled, internal units can make great strides towards a big change in the overall output of
company.

That being said, I decided to join of these big machines within Telco in hope of facing challenging data problems.
...
from boring and flaky tasks like integrations between systems, retries on connection errors 
...
Although short-lived, some data products that we made were quite powerful 
...
Plus I hope to have spread the bug to others.
...
a company's ability to deliver software products in the process of growing their business.
...
However, my attitude needs the right vehicle to flourish.
...
No more databases returning mysterious nulls or empty strings or other nonsensical output.
For example, a call to the database gives back a type for all parties involved - 
...
but in return you get confidence and productivity gains that every employee and manager appreciates.
...
With a correct blend of OO and FP, Scala allows for elegant solutions to these types of problems.
By elegant I mean succinct and concrete.
For example. take a look at what Scala 3 [enums](https://dotty.epfl.ch/docs/reference/enums/enums.html) 
can [do](https://dotty.epfl.ch/docs/reference/enums/adts.html).
...
With the evolution of the language, comes the evolution of the individual.
Through Scala, and it's application in Spark, for example, I have learned much about distributed systems, 
encoding formats, batch and stream processing among many others.
Not just the concepts but the actual implementation details.

### JVM-native
Software doesn't exist in isolation.
Scala's ability to have a seamless integration with Java code is simply overlooked.
...
### Robust
What happens after all the trails and tribulations, 
developers who struggle with Scala in one way or the other,
...
For example, I used Akka libraries in many projects. 
Once the initial bug fixing cycle was completed, 
the only time I revisited my implementation was to address business needs.
...
It is about how your team's knowledge and experience with Scala and FP.
...
[Here](https://www.quora.com/Why-isnt-Scala-backward-compatible?share=1) is a great writeup of why Scala historically has
broken minor (the y in x.y.z) version compatibility. The gist of it is:
...
Martin Odersky has referenced the [Principle of Least Power](https://www.lihaoyi.com/post/StrategicScalaStylePrincipleofLeastPower.html)
...
In other words focusing on solving business problems and not monads.

In this category also falls the deployment of Scala applications.
Historically those have been jar files and now wrapped in docker files.
...
allowing CI/CD pipelines ot run faster.
...
that Databricks follows by promote Python as the go-to language for data-engineering seems wrong.
...
This [article](https://itnext.io/making-the-spark-dataframe-composition-type-safe-r-7b6fed524ec2) gives a good idea of
...
This is just one example of how Scala can power data to a new level.

# Conclusion
In this post I gave my perspective on Scala.
I started by presenting a backstory of my professional work relating to the language.
I proceeded to share my good and bad experiences using Scala.
Based on those I was bold enough to give some recommendations to fellow Scala developers.
...
The first question mark for success revolves around tooling.
If the IDEs and the major libraries in the ecosystem are there to take the first wave of transition, 
all will be well.
Another question mark are the big companies using Scala - I am talking about Spark, of course.
It has been [trying](https://issues.apache.org/jira/browse/SPARK-25075) to go to Scala 2.13 for a while.
